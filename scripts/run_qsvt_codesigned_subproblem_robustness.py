...
from robust_qsvt_se.qsvt.codesigned_subproblem_robustness import (  # noqa: E402
    DEFAULT_ALPHAS,
    DEFAULT_DEGREES,
    DEFAULT_SELECTION_MODES,
    DEFAULT_TARGET_FAMILIES,
    run_qsvt_codesigned_subproblem_robustness,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Robustness of co-designed QSVT targets across criteria-selected subproblems."
    )
    ...
    parser.add_argument("--output-dir", default="outputs/qsvt_codesigned_subproblem_robustness")
...
def main() -> None:
    ...
    run = run_qsvt_codesigned_subproblem_robustness(config)
    rows = run["rows"]
    ...
    print(f"Wrote subproblem robustness study to {run['output_dir']}")
